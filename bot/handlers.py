...
class BotHandlers:
    ...
    async def submit_form_handler(self, message: Message):
        """Обрабатывает нажатие кнопки "Отправить заявку" """
        user_id = message.from_id
        ...
        # Создание заявки теперь асинхронное
        ticket_id = await self.form_handler.create_ticket(user_id)
        ...
        if ticket_id:
            ...
            logger.error(
                f"Failed to submit form for user {user_id}. DB error occurred."
            )
            await message.answer(
    ...
    async def list_tickets_handler(self, message: Message):
        ...
        tickets = await self.db_handler.get_all_tickets(user_id)
    ...
    async def view_ticket_handler(self, message: Message):
        ...
        if not ticket_id:
            logger.warning(
                f"View ticket command from user {user_id} without ticket_id in payload."
            )
            await message.answer(
        ...
        ticket = await self.db_handler.get_ticket(ticket_id)

        if not ticket or ticket["user_id"] != user_id:
            logger.warning(
                f"User {user_id} tried to view unauthorized or "
                f"non-existent ticket {ticket_id}"
            )
            await message.answer(
        ...
        created_at_dt = datetime.fromisoformat(ticket["created_at"])
        ticket_info += (
    ...
    async def prompt_ticket_deletion(self, message: Message, ticket_id: str):
        """
        Вспомогательный метод: проверяет заявку и запрашивает подтверждение
        удаления.
        """
        user_id = message.from_id
        logger.info(
    ...
    async def delete_ticket_prompt_handler(self, message: Message):
        """Запрашивает подтверждение удаления заявки."""
        payload = json.loads(message.payload)
        ticket_id = payload.get("ticket_id")
        await self.delete_ticket_confirm_handler(message, ticket_id)

    async def delete_ticket_confirm_handler(
        self, message: Message, ticket_id_from_trigger: Optional[str] = None
    ):
        """
        ...
        ticket_id_from_state = self.form_handler.get_user_state(
            user_id, "ticket_to_delete"
        )
        logger.info(
            f"Confirm delete triggered for ticket "
            f"{ticket_id_from_trigger} (state: {ticket_id_from_state})"
            f" by user {user_id}"
        )

        if not ticket_id_from_state or (
            ticket_id_from_trigger and ticket_id_from_trigger != ticket_id_from_state
        ):
            logger.warning(
                f"Delete confirm mismatch or missing state for user "
                f"{user_id}. Trigger: {ticket_id_from_trigger}, "
                f"State: {ticket_id_from_state}"
            )
            await message.answer(
        ...
        success = await self.form_handler.delete_ticket(user_id, ticket_id)
        self.form_handler.clear_user_state(user_id, "ticket_to_delete")
    ...
    async def cancel_action_handler(self, message: Message):
        """Обработчик кнопки "Отмена" в различных контекстах"""
        user_id = message.from_id
    ...
    async def form_message_handler(self, message: Message):
        ...
        if user_id not in self.form_handler.user_forms:
            logger.debug(
                f"Text message '{message.text}' from user {user_id} "
                f"received, but not filling form. Ignoring."
            )
            await self.default_handler(message)
            return

        logger.info(f"Processing form answer '{message.text}' from user {user_id}")
        next_question_or_status = self.form_handler.process_answer(
    ...
    async def default_handler(self, message: Message):
        ...
        Пытается обработать сообщение как индекс заявки, "Удалить заявку"
        или "Подтвердить удаление".
        """
        user_id = message.from_id
        message_text = message.text.strip()
        logger.info(
            f"Default handler caught message '{message_text}' from user {user_id}"
        )
        message_text_lower = message_text.lower()

        if message_text_lower == "удалить заявку":
            last_viewed_ticket = self.form_handler.get_user_state(
                user_id, "last_viewed_ticket"
            )
            if last_viewed_ticket:
                logger.info(
                    f"Interpreting text 'Удалить заявку', prompting "
                    f"deletion for last viewed ticket "
                    f"{last_viewed_ticket}"
                )
                await self.prompt_ticket_deletion(message, last_viewed_ticket)
                return
            else:
                logger.warning(
                    f"User {user_id} sent 'Удалить заявку' text but"
                    f" no last viewed ticket found in state."
                )
                await message.answer(
                    "Пожалуйста, сначала выберите заявку для удаления из "
                    "списка 'Мои заявки'.",
                    keyboard=keyboards.get_start_keyboard(),
                )
            return
            ...
            ticket_to_delete = self.form_handler.get_user_state(
                user_id, "ticket_to_delete"
            )
            if ticket_to_delete:
                logger.info(
                    f"Interpreting text 'Подтвердить удаление', "
                    f"confirming deletion for ticket {ticket_to_delete}"
                )
                await self.delete_ticket_confirm_handler(message, ticket_to_delete)
                return
            else:
                logger.warning(
                    f"User {user_id} sent 'Подтвердить удаление' "
                    f"text but no ticket_to_delete found in state."
                )
            ...
            try:
                ...
                if 0 <= index < len(cached_tickets):
                    ...
                    logger.info(
                        f"Interpreting message as index {index + 1}, "
                        f"viewing ticket {ticket_id}"
                    )
                    await self.show_ticket_details(message, ticket_id)
                    return
                else:
                    logger.warning(
                        f"User {user_id} entered invalid ticket index: {index + 1}"
                    )
            except ValueError:
                logger.error(
                    f"ValueError converting message text "
                    f"'{message_text}' to int for user {user_id}"
                )
            except Exception as e:
                logger.error(
                    f"Error processing message as ticket index for user {user_id}: {e}"
                )
    ...
    async def notify_admins_about_new_ticket(
        self, ticket_id: str, user_id: int, form_data: Dict
    ):
